...
def grid_points(img, nPointsX, nPointsY, border):
    ...
    # Compute the spacing between grid points while considering the border
    spacingX = (w - 2 * border) / (nPointsX - 1)
    spacingY = (h - 2 * border) / (nPointsY - 1)

    # Generate grid points
    for ix in range(nPointsX):
        for iy in range(nPointsY):
            x = border + ix * spacingX
            y = border + iy * spacingY
            vPoints.append((x, y))

    vPoints = np.array(vPoints)  # Convert list of tuples to numpy array
    return vPoints
...
def descriptors_hog(img, vPoints, cellWidth, cellHeight):
    ...
    angles[angles < 0] += 360  # Convert negative angles to positive
    ...
    for i in range(len(vPoints)):
        ...
        for cell_y in range(-2, 2):
            for cell_x in range(-2, 2):
                ...
                # todo
                # Extract the cell's gradient orientations
                cell_angles = angles[start_y:end_y, start_x:end_x]

                # Calculate histogram
                hist, _ = np.histogram(cell_angles, bins=nBins, range=(0, 360))
                desc.extend(hist)
...
def create_codebook(nameDirPos, nameDirNeg, k, numiter):
    ...
    for i in tqdm(range(nImgs)):
        ...
        # Compute descriptors for each grid point
...
def bow_histogram(vFeatures, vCenters):
    ...
    """
    # Use the findnn function to get the indices of the nearest centroid for each feature
    indices, _ = findnn(vFeatures, vCenters)

    # Initialize histogram with zeros
    nCenters = vCenters.shape[0]
    histo = np.zeros(nCenters)
...
def bow_recognition_nearest(histogram,vBoWPos,vBoWNeg):
    ...
    DistPos, DistNeg = distPos[0], distNeg[0]
