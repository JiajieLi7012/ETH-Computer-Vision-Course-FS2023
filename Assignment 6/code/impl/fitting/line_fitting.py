...
def num_inlier(x,y,k,b,n_samples,thres_dist):
	...
	# Distance from a point (x_i, y_i) to the line y = kx + b:
	# d = |kx_i - y_i + b| / sqrt(k^2 + 1)
	num = 0
	mask = np.zeros(n_samples, dtype=bool)
	for i in range(n_samples):
		d = abs(k*x[i] - y[i] + b) / np.sqrt(k**2 + 1)
		# Inlier if d < thres_dist
		if d < thres_dist:
			num += 1
			mask[i] = True
	return num, mask
...
def ransac(x,y,iter,n_samples,thres_dist,num_subset):
	...
	for i in range(iter):
		...
		idx = random.sample(range(n_samples), num_subset)
		x_subset = x[idx]
		y_subset = y[idx]
