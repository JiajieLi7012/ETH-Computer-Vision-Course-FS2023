...
class SegNetLite(nn.Module):
    ...
    def __init__(self, kernel_sizes=[3, 3, 3, 3], down_filter_sizes=[32, 64, 128, 256],
        ...
        for i in range(self.num_down_layers):
            layers_conv_down.append(nn.Conv2d(input_size, down_filter_sizes[i], 
            ...
            input_size = down_filter_sizes[i] # Update input size for the next layer
        ...
        input_size = 256  # Start with the output size of the last downsampling layer
        for i in range(self.num_down_layers):
            layers_unpooling.append(nn.MaxUnpool2d(pooling_kernel_sizes[i], stride=pooling_strides[i]))
            layers_conv_up.append(nn.Conv2d(input_size, up_filter_sizes[i], 
                                            kernel_size=kernel_sizes[i], padding=conv_paddings[i]))
            layers_bn_up.append(nn.BatchNorm2d(up_filter_sizes[i]))
            input_size = up_filter_sizes[i]  # Update input size for the next layer
    ...
    def forward(self, x):
        ...
        # Upsampling
        for i in range(self.num_down_layers):
            x = self.layers_unpooling[i](x, indices_list[self.num_down_layers - 1 - i])
